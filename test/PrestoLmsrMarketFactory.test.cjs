...
async function deployFactoryFixture() {
  ...
  const factory = await Factory.connect(owner).deploy(await collateral.getAddress());
  return { collateral, factory, owner, creator, resolver, feeRecipient, nextOwner, outsider };
}

describe('PrestoLmsrMarketFactory', function () {
  it('stores owner, collateral, and defaults', async function () {
    ...
    expect(await factory.defaultBond6()).to.equal(0);
    expect(await factory.marketCount()).to.equal(0);
  });

  it('creates an LMSR market carrying the current fee, bond, and guardian', async function () {
    ...
    await factory.connect(owner).setFees(feeRecipient.address, 150);
    await factory.connect(owner).setDefaultBond(USDC(10));

    const tx = await factory.connect(creator).createMarket(
      resolver.address, closeTime, 'ipfs://market-metadata', 0, 3, USDC(100),
    );
    ...
    expect(event.args.marketKind).to.equal(0);
    expect(event.args.closeTime).to.equal(closeTime);
    expect(event.args.metadataURI).to.equal('ipfs://market-metadata');

    const market = await ethers.getContractAt('PrestoLmsrMarket', marketAddress);
    ...
    expect(await market.protocolFeeRecipient()).to.equal(feeRecipient.address);
    expect(await market.feeBps()).to.equal(150);
    expect(await market.outcomeCount()).to.equal(3);
    expect(await market.closeTime()).to.equal(closeTime);
    expect(await market.metadataURI()).to.equal('ipfs://market-metadata');
    expect(await market.bond6()).to.equal(USDC(10));
    expect(await market.guardian()).to.equal(owner.address);
  });

  it('restricts fee and bond controls to the owner', async function () {
    ...
    await expect(factory.connect(outsider).setDefaultBond(USDC(5))).to.be.revertedWithCustomError(factory, 'NotOwner');

    await expect(factory.setFees(feeRecipient.address, 100)).to.emit(factory, 'FeesUpdated').withArgs(feeRecipient.address, 100);
    await expect(factory.setDefaultBond(USDC(5))).to.emit(factory, 'DefaultBondUpdated').withArgs(USDC(5));
    expect(await factory.protocolFeeBps()).to.equal(100);
    expect(await factory.defaultBond6()).to.equal(USDC(5));
  });
  ...
  it('restricts ownership transfer to the owner', async function () {
    ...
    await expect(factory.connect(owner).transferOwnership(nextOwner.address))
      .to.emit(factory, 'OwnershipTransferred').withArgs(owner.address, nextOwner.address);

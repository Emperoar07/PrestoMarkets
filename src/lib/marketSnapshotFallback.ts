...
const TMP_SNAPSHOT_PATH = path.join(os.tmpdir(), 'presto-market-snapshot.json');
// Committed at repo root under data/. cwd is the project root in both dev and the Vercel runtime.
const SEED_SNAPSHOT_PATH = path.join(process.cwd(), 'data', 'markets-seed.json');
...
export function writeFilesystemSnapshot(markets: AppMarket[]): void {
  if (!isServer || markets.length === 0) return;
  try {
    const slim = slimSnapshotForStorage(markets);
    fs.writeFileSync(TMP_SNAPSHOT_PATH, JSON.stringify({ markets: slim, at: Date.now() }));
  } catch {
...
export function readFilesystemSnapshot(): FallbackSnapshot | null {
  ...
  try {
    const raw = fs.readFileSync(TMP_SNAPSHOT_PATH, 'utf8');

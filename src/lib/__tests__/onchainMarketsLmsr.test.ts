...
describe('getLmsrOdds', () => {
  it('maps WAD prices to whole-percent odds that sum to 100', () => {
    ...
    expect(getLmsrOdds([WAD / BigInt(2), WAD / BigInt(2)])).toEqual([50, 50]);
  });

  it('reflects a skewed book and still sums to 100', () => {
    // 0.7 / 0.3 split.
    const odds = getLmsrOdds([(WAD * BigInt(7)) / BigInt(10), (WAD * BigInt(3)) / BigInt(10)]);

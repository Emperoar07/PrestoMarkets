...
export async function agentCreateMarket(input: CreateLiveMarketInput & { agentResolverAddress?: string }) {
  try {
    ...
    if (lmsrFactoryAddress) {
      ...
      const receipt = await publicClient.waitForTransactionReceipt({ hash: createHash, timeout: 90_000, pollingInterval: 1_000 });
      ...
      if (marketAddress) {
        ...
        try {
          ...
          if (allowance < seed6) {
            ...
            await publicClient.waitForTransactionReceipt({ hash: approveHash, timeout: 90_000, pollingInterval: 1_000 });
          }
          ...
          await publicClient.waitForTransactionReceipt({ hash: seedHash, timeout: 90_000, pollingInterval: 1_000 });

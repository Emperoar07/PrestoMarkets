...
async function main() {
  ...
  console.log('========================================');
  console.log('PrestoMarketFactory Deployment');
  console.log('========================================');
  ...
  const Factory = await ethers.getContractFactory('PrestoMarketFactory');
  ...
  const deploymentPath = path.join(dataDir, 'arc-testnet.json');
  fs.mkdirSync(dataDir, { recursive: true });
  fs.writeFileSync(deploymentPath, JSON.stringify(deployment, null, 2));

  console.log('PrestoMarketFactory:', factoryAddress);
